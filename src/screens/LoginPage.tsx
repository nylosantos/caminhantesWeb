...
export function LoginPage() {
  // GET GLOBAL DATA
  const { auth, login, setIsSubmitting, handleUser, setLogin } = useContext(
    GlobalDataContext
  ...
  const signInWithSocial = async (provider: 'twitter' | 'google') => {
    ...
    if (socialProvider !== undefined) {
      const result = await signInWithPopup(auth, socialProvider).catch((error) => {
        console.log(error);
        toast.error('Não foi possível logar com sua rede social? Tente com e-mail e senha...');
      ...
      if (result) {
        ...
        await createDbUser({
          ...
          .catch((error) => {
            console.log(error);
            toast.error('Erro ao verificar usuário no banco de dados. Tente com e-mail e senha...');
            setIsSubmitting(false);
          });
        // setIsSubmitting(false);
      } else return;
  ...
  async function handleSignUpWithEmailAndPassword() {
    ...
    if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(loginData.login)) {
    ...
    if (errors.length > 0) {
      ...
      // SIGN UP FUNCTION
      await createUserWithEmailAndPassword(auth, loginData.login, loginData.password)
  ...
  return (
    <Container>
      <div className="flex h-full w-full flex-col items-center justify-center gap-4">
        ...
        <Button
          buttonFunction={() =>
            login ? handleSignInWithEmailAndPassword() : handleSignUpWithEmailAndPassword()
          }
          type="login"
